...
void SetFilename(){
  ...
  //printf("%s\n",FILENAME[count_file-1]);
}

void SetFilename2(){
  int i,j;
  dir2 = opendir(ddirname2);
  if(dir2==NULL){
    printf("unko2\n");
    exit(0);
  }
  for(i = 0; NULL != (dp2 = readdir(dir2)); i++){
    strcpy(FILENAME2[i],dp2->d_name);
    count_file2++;
  }
  char temp[MAX_CHAR_NUM];
  for(i =0; i < count_file2-1; i++){
    for(j = i+1; j < count_file2; j++){
      if ((strcmp(FILENAME2[j],FILENAME2[i])) < 0){
	strcpy(temp,FILENAME2[i]);
	strcpy(FILENAME2[i],FILENAME2[j]);
	strcpy(FILENAME2[j],temp);
      }
    }
  }
  //printf("%s\n",FILENAME[count_file-1]);
...
void ReadTIFFileMaskforSF(){
  ...
  for(slice = 0; slice < slice_tot; slice++){
    ...
    for (y=0;y<(int)rows; y++){
      for (x=0;x<(int)columns;x++){
	if(in_image[y*columns+x]==0){
	  if(slice>OFS_MIN_Z&&slice<OFS_MAX_Z&&
	     y>OFS_MIN_Y&&y<OFS_MAX_Y&&
	     x>OFS_MIN_X&&x<OFS_MAX_X){
	    d_mask[Indix(slice,y,x)]=0;
	  }
	  else{
	    d_mask[Indix(slice,y,x)]=1;
	  }
	}
	else{
	  d_mask[Indix(slice,y,x)]=1;
